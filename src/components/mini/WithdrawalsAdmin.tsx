...
export function WithdrawalsAdmin({ tenantId, initData, previewTgId }: Props) {
  ...
  const m = useMutation({
    mutationFn: (v: { txId: string; approve: boolean }) =>
      process({ data: { ...auth, ...v, reason: v.approve ? null : reason || null } }),
    onSuccess: (r: any) => {
            ...
            open === r.id ? (
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-white/40">
                  Approve sends {String(r.network || "").toLowerCase().includes("ton") ? "GRAM from your Tonkeeper phrase" : "USDT from the matching EVM private key"} and waits for blockchain confirmation.
                </p>
                <div className="flex gap-2">
                  <Button size="sm" className="flex-1" disabled={m.isPending} onClick={() => m.mutate({ txId: r.id, approve: true })}>{m.isPending ? "Sending…" : "Approve & send"}</Button>
